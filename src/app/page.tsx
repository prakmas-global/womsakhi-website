...
function Hero() {
  return <><section className="hero" id="home"><div className="heroCopy"><p className="eyebrow">EMPOWER · EDUCATE · ELEVATE</p><h1>Every Woman<br/>Has a <em>Stronger</em><br/>Tomorrow</h1><p className="lead">WomSakhi is a supportive platform empowering women with knowledge, opportunities, and a caring community to build independent and fulfilling lives.</p><div className="actions"><a className="primary" href={`${APP}/signup`}>Join the Movement　→</a><button className="watch"><i>▶</i> Watch Our Story</button></div><div className="proof"><span className="faces">{[15,48,80].map(x=><Image key={x} src="/assets/community.png" alt="" width={42} height={42} style={{objectPosition:`${x}% center`}}/>)}</span><span><b>10K+</b><small>Women are already on this journey</small></span></div></div><div className="heroArt referenceArt"><Image src="/reference/hero.png" alt="A confident woman looking toward a brighter future" fill priority sizes="621px"/><a className="artHotspot" href="#programs" aria-label="Explore small steps and big changes"/></div></section><section className="stats">{[["♙","10K+","Women Empowered"],["▤","200+","Skill Programs"],["◇","50+","Partner Organizations"],["♡","95%","Positive Impact"]].map(s=><article key={s[1]}><i>{s[0]}</i><b>{s[1]}</b><small>{s[2]}</small></article>)}</section></>;
}

function Mission() {
  return <section className="mission" id="mission"><div className="missionArt referenceArt"><Image src="/reference/mission.png" alt="Women supporting one another at sunset" fill sizes="530px"/><a className="artHotspot" href="#programs" aria-label="Learn more about the WomSakhi mission"/></div><div className="missionCopy"><p className="eyebrow berry">OUR MISSION</p><h2>A More Equal,<br/>Inclusive and <em>Empowered</em><br/>World for Women</h2><p>We believe in a world where every woman has access to education, opportunities, support and a community that stands with her. WomSakhi bridges gaps, opens doors and creates real change.</p><div className="pillars">{[["▤","Education","& Learning"],["♙","Career","Opportunities"],["♧","Community","Support"],["♡","Health","& Wellbeing"]].map(p=><span key={p[1]}><i>{p[0]}</i>{p[1]}<br/>{p[2]}</span>)}</div><a className="textLink" href="#programs">Learn more about our mission <b>→</b></a></div></section>;
}

function Offers() {
  return <section className="offers" id="programs"><div className="sectionHead"><div><p className="eyebrow berry">WHAT WE OFFER</p><h2>Tools for a Brighter Tomorrow</h2></div><a href="#cards">Explore all programs →</a></div><div className="cards referenceCards" id="cards">{offers.map((o,i)=><article key={o[0]}><Image src={`/reference/card-${["skill","career","community","wellness"][i]}.png`} alt={o[0]} fill sizes="209px"/><a href={`${APP}/app/${o[3]}`} aria-label={`Explore ${o[0]}`}/></article>)}</div></section>;
}

export default function Home() {
  return <div className="site"><Header/><main><Hero/><Mission/><Offers/><section className="closing referenceClosing"><Image src="/reference/closing.png" alt="Together we create brighter tomorrows" fill priority sizes="1007px"/><a href={`${APP}/signup`} aria-label="Join WomSakhi today"/></section></main></div>;
}
