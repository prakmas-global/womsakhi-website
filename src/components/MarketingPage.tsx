...
export function MarketingHeader() {
  ...
  return <header className="header marketingHeader"><Link className="brand brandOfficial" href="/"><Image src="/assets/womsakhi-lotus.png" alt="" width={48} height={32} priority/><span><b>WomSakhi</b><small>STRONGER WOMEN. BRIGHTER TOMORROWS.</small></span></Link><nav className={open?"open":""}>{navigation.map(([label,href])=><Link onClick={()=>setOpen(false)} key={href} href={href}>{label}</Link>)}</nav><div className="headerActions"><a className="searchAction" aria-label="Search WomSakhi" href={`${APP}/app/search`}>⌕</a><a className="ghost" href={`${APP}/signin`}>Sign In</a><a className="primary" href={`${APP}/signup`}>Get Started</a></div><button className="menu" aria-label="Open navigation" aria-expanded={open} onClick={()=>setOpen(v=>!v)}>☰</button></header>;
}

function SiteFooter() {
  return <footer className="marketingFooter"><div className="footerBrand"><div className="footerLogo"><Image src="/assets/womsakhi-lotus.png" alt="" width={54} height={36}/><b>WomSakhi</b></div><p>Learn. Work. Earn. Belong.</p></div><div><b>Explore</b><Link href="/programs">Programs</Link><Link href="/community">Community</Link><Link href="/resources">Resources</Link></div><div><b>Support</b><a href={`${APP}/app/help`}>Help Centre</a><a href={`${APP}/app/safety`}>Safety Centre</a><Link href="/contact">Contact</Link></div><div><b>Join</b><a href={`${APP}/signup`}>Create an account</a><a href={`${APP}/signin`}>Member sign in</a><p>Made for women, with women.</p></div></footer>;
}
